---
title: "Modelagem do Prêmio frequência-severidade por Bullman-Straub, Random Forest e XGboost com grupos tarifários agrupados por experiência do analista"
output:
...
# ==========================================
# AGRUPANDO POR GRUPOS TARIFARIOS - Analista
# ==========================================
```{r}
grupoAlt = unique((df_regiao %>% filter(Grupo_Tarifario == "Alto"))$Regiao) # alto indice de colisão
grupoMed = unique((df_regiao %>% filter(Grupo_Tarifario == "Medio"))$Regiao) # medio indice de colisão
grupoBai = unique((df_regiao %>% filter(Grupo_Tarifario == "Baixo"))$Regiao) # baixo indice de colisão
...
# ===========================================
```{r}
grupoAlt = unique((df_regiao %>% filter(Grupo_Tarifario == "Alto"))$Regiao) # alto indice de roubos
grupoMed = unique((df_regiao %>% filter(Grupo_Tarifario == "Medio"))$Regiao) # medio indice de roubos
grupoBai = unique((df_regiao %>% filter(Grupo_Tarifario == "Baixo"))$Regiao) # baixo indice de roubos
...
### Versão Bayesiana do modelo de Bühlmann-Straub considerando a experiência do analista
#======================
#===== FREQUÊNCIA =====
#======================
...
Find = colSums(n)/colSums(v) ### frequencia do numero de sinistros (obitos)
```
```{r}
...
### metrica de comparação dos modelos
...
Conclusão: Ao realizar um teste com modelos de aprendizado supervisionado considerando algoritmos como o Random Forest e o XGBoost, nota-se a partir do gráfico que os dois modelos superestimam bastante o premio frequência-severidade comparado aos dados reais para o Grupo de Alto Risco, entretanto, os modelo de Bullman-Straub e XGBoost aproximaram-se muito mais dos dados observados para o Grupo Médio que o modelo Random Forest. Para o Grupo Tarifário Baixo o modelo XGboost estimou valores de premios frequência-severidade mais próximos dos valores observados em comparação aos modelos Bullman-Straub e Random Forest que superestimaram os prêmios.

O erro quadrático médio foi maior para os modelos de Bullman-Straub e Random Forest e menor para o XGboost. Os valores podem ser observados na Tab_RMSE.